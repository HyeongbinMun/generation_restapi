...
class FaceAdapter:
    def __init__(self, crop_ratio=0.81, cache_dir='./hub',
                 base_model='CompVis/stable-diffusion-v1-4', use_cache=False):
        self.checkpoint = '/workspace/model/FaceAdapter/checkpoints'
        self.output = '/workspace/images'
    ...
    def _initialize_paths(self):
        ...
        self.save_drive_path = os.path.join(self.output, 'drive')
        os.makedirs(self.save_drive_path, exist_ok=True)
        self.save_swap_path = os.path.join(self.output, 'swap')
        os.makedirs(self.save_swap_path, exist_ok=True)
        self.save_concat_path = os.path.join(self.output, 'concat')
        os.makedirs(self.save_concat_path, exist_ok=True)

    def _load_models(self):
        # Load all required models
        self.controlnet = ControlNetModel.from_pretrained(
            os.path.join(self.checkpoint, 'controlnet'),
        ...
        pretrained_unet_path = os.path.join(self.checkpoint, 'pretrained_unet')
        if os.path.exists(pretrained_unet_path) and 'stable-diffusion' in self.base_model:
            self.pipe.unet = UNet2DConditionModel.from_pretrained(pretrained_unet_path,
        ...
        self.pipe.vae = vae_ft_mse

        # Load additional models
        self.net_d3dfr = model_resnet_d3dfr.getd3dfr_res50(
            os.path.join(self.checkpoint, 'third_party/d3dfr_res50_nofc.pth')).eval().to(self.device)
        self.bfm_facemodel = bfm.BFM(
            focal=1015 * 256 / 224,
            image_size=256,
            bfm_model_path=os.path.join(self.checkpoint, 'third_party/BFM_model_front.mat')
        ).to(self.device)
        self.net_arcface = model_insightface_backbone.getarcface(
            os.path.join(self.checkpoint, 'third_party/insightface_glint360k.pth')
        ).to(self.device)

        self.clip_image_processor = CLIPImageProcessor()
        self.net_vision_encoder = CLIPVisionModel.from_pretrained(
            os.path.join(self.checkpoint, 'vision_encoder')
        ).to(self.device)
        ...
        self.net_image2token.load_state_dict(torch.load(os.path.join(self.checkpoint, 'net_image2token.pth')))

        self.net_id2token = ID2Token(id_dim=512, text_hidden_size=768, max_length=77, num_layers=3).to(self.device)
        self.net_id2token.load_state_dict(torch.load(os.path.join(self.checkpoint, 'net_id2token.pth')))

        self.net_seg_res18 = model_seg_unet.UNet().eval().to(self.device)
        self.net_seg_res18.load_state_dict(torch.load(os.path.join(self.checkpoint, 'net_seg_res18.pth')))

        self.app = FaceAnalysis(name='antelopev2', root=os.path.join(self.checkpoint, 'third_party'),
                                providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        self.app.prepare(ctx_id=0, det_size=(640, 640))

    def inference(self, src_img_path, drive_img_path):
        src_file = os.path.basename(src_img_path)
        src_im_name = src_file.split('.')[0]
        drive_file = os.path.basename(drive_img_path)
        drive_im_name = drive_file.split('.')[0]
        src_im_pil = Image.open(src_img_path).convert("RGB")
        out_images = []

        # ===== insightface detect 5pts
        face_info = self.app.get(cv2.cvtColor(np.array(src_im_pil), cv2.COLOR_RGB2BGR))
        ...
        clip_input_src_tensors = self.clip_image_processor(images=src_im_pil, return_tensors="pt").pixel_values.view(-1, 3, 224, 224).to(device)

        drive_im_pil = Image.open(drive_img_path).convert("RGB")

        # ===== insightface detect 5pts
        face_info = self.app.get(cv2.cvtColor(np.array(drive_im_pil), cv2.COLOR_RGB2BGR))
        face_info = \
        ...
        warp_mat_crop = datasets_faceswap.transformation_from_points(bbox_pts4,
                                                                     datasets_faceswap.mean_box_lm4p_512)
        drive_im_crop512 = cv2.warpAffine(np.array(drive_im_pil), warp_mat_crop, (512, 512),
        ...
        clip_input_tar_tensors = self.clip_image_processor(images=drive_im_pil,
                                                      return_tensors="pt").pixel_values.view(-1, 3, 224,
                                                                                             224).to(device)
        ...
        im_pts70 = draw_pts70_batch(gt_pts68, gt_d3d_coeff[:, 257:], image_tar_warpmat256, test_image_size,
                                    return_pt=True)
        im_pts70 = im_pts70.to(images_tar)
        face_masks_tar = (self.net_seg_res18(torch.cat([images_src, im_pts70], dim=1)) > 0.5).float()
        controlnet_image = im_pts70 * face_masks_tar + images_src * (
                    1 - face_masks_tar)  # tar for reconstruction
        ...
        faceid = self.net_arcface(F.interpolate(image_src_crop256, [128, 128], mode='bilinear'))
        encoder_hidden_states_src = self.net_id2token(faceid).to(dtype=weight_dtype)

        last_hidden_state = self.net_vision_encoder(clip_input_src_tensors).last_hidden_state
        controlnet_encoder_hidden_states_src = self.net_image2token(last_hidden_state).to(dtype=weight_dtype)

        last_hidden_state = self.net_vision_encoder(clip_input_tar_tensors).last_hidden_state
        controlnet_encoder_hidden_states_tar = self.net_image2token(last_hidden_state).to(dtype=weight_dtype)

        empty_prompt_token = torch.load('empty_prompt_embedding.pth').view(1, 77, 768).to(
            dtype=weight_dtype).to(device)
        ...
        images_res_pil = Image.fromarray(
            (res_tensor[0] * 127.5 + 128).cpu().numpy().astype('uint8').transpose(1, 2, 0))

        # save
        # images_res_pil.save(os.path.join(self.save_drive_path, src_file), quality=100)
        out_images.append(images_res_pil)

        ##########  face reenactment  ##########

        ##########    face swapping   ##########
        net_seg = model_parsing.get_face_parsing(
            os.path.join(self.checkpoint, 'third_party/79999_iter.pth')).eval().to(device)
        seg_pred = net_seg(images_tar)[0]
        masks_tar = torch.argmax(
            F.interpolate(seg_pred, [test_image_size, test_image_size], mode='bilinear'), dim=1,
            keepdim=True)  # torch.Size([1, 1, 512, 512]) 0-20
        ...
        im_rgb_pil = Image.fromarray(
            convert_batch_to_nprgb(torch.cat([images_src, images_tar, res_tensor, swap_res_tensor]), 4))

        # save
        im_rgb_pil.save(os.path.join(self.save_concat_path, f'{src_im_name}_{drive_im_name}.jpg'), quality=100)
        out_images.append(im_rgb_pil)

        images_res_pil = Image.fromarray((swap_res_tensor[0] * 127.5 + 128).cpu().numpy().astype('uint8').transpose(1, 2, 0))

        # save
        images_res_pil.save(os.path.join(self.save_swap_path, src_file), quality=100)
        out_images.append(images_res_pil)

        return out_images

# face = FaceAdapter()
# face.inference('/workspace/images/1.jpg', '/workspace/images/2.jpg')
