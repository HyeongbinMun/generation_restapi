...
z = torch.from_numpy(np.random.RandomState(config.seed_canvas).randn(2, G_canvas.z_dim)).to(device)
print("canvas z shape:", z.shape)
with torch.no_grad():
    random_humans_w = G_canvas.mapping(z, None)
    print("random_humans_w shape:", random_humans_w.shape)
...
face_latents_path = "/workspace/encoder4editing/weights/latents.pt"
face_latents = torch.load(face_latents_path)

if isinstance(face_latents, torch.Tensor):
    print("Latents shape:", face_latents.shape)

face_latents = face_latents.to("cuda")

# z = torch.from_numpy(np.random.RandomState(config.seed_inset).randn(1, G_inset.z_dim)).to(device)
# print("z shape:", z.shape)
with torch.no_grad():
    random_face_w = G_inset.mapping(face_latents, None)
